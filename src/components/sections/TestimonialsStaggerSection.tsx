...
/**
 * Duplicate of TestimonialsSection using a staggered, overlapping card deck
 * instead of the auto-scrolling marquee — for side-by-side comparison.
 * Adapted from an externally supplied shadcn-style reference component
 * (notched corners, fanned rotation, click-to-center): this project has no
 * shadcn/ui or lucide-react, so the cn() helper, hsl(var(--token)) colors,
 * and pravatar.cc headshots were swapped for this project's own tokens,
 * Phosphor icons, and initials avatars (no fabricated stock photos, see
 * pitfalls.md #7).
 *
 * Cards are keyed by a stable slot index into a fixed, tripled testimonials
 * array — never regenerated — so moving the active index just recomputes
 * each card's `position` and CSS transitions the existing DOM node to its
 * new spot. That's what makes clicking a card slide it smoothly into the
 * center instead of instantly swapping content in place. The active index
 * is kept in the middle copy and silently rewound by one loop length a beat
 * after each move (transition off for that one frame) for infinite runway.
 */
...
export function TestimonialsStaggerSection() {
  ...
  return (
    <section id="testimonials-stagger" className="border-b border-border-primary pb-16 md:pb-20">
